...
ShadowMapPass::ShadowMapPass(Microsoft::WRL::ComPtr<ID3D12Device>& Dev, std::string ShaderPath, int Priority) :PassRenderer(Dev, ShaderPath, Priority, PassRenderer::PassType::SHADER)
{
	Init(ShaderPath);
}
...
void ShadowMapPass::PassClearAndSetRendererTarget(Microsoft::WRL::ComPtr<ID3D12GraphicsCommandList>& _CmdList, Microsoft::WRL::ComPtr<ID3D12DescriptorHeap>& _DepthStencilViewHeap, bool First)
{
	auto HeapPointer = _DepthStencilViewHeap->GetCPUDescriptorHandleForHeapStart();
	HeapPointer.ptr+=  _Dev->GetDescriptorHandleIncrementSize(D3D12_DESCRIPTOR_HEAP_TYPE_DSV);
	_CmdList->OMSetRenderTargets(0,nullptr, false, &HeapPointer);
	_CmdList->ClearDepthStencilView(HeapPointer, D3D12_CLEAR_FLAG_DEPTH, 1.0f, 0, 0, nullptr);
}

void ShadowMapPass::Draw(Microsoft::WRL::ComPtr<ID3D12GraphicsCommandList>& _CmdList, D3D12_VIEWPORT & _ViewPort, D3D12_RECT & _ScissorRect, Microsoft::WRL::ComPtr<ID3D12DescriptorHeap>& _DepthBufferViewHeap)
{
	//ルートシグネチャをセット
...
bool ShadowMapPass::CreatePipeLineState()
{
	HRESULT Result = S_OK;
...
bool ShadowMapPass::Create_Rtv_Srv()
{
	HRESULT Result = S_OK;
