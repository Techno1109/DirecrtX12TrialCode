...
RayMarchingPass::RayMarchingPass(Microsoft::WRL::ComPtr<ID3D12Device>& Dev, std::string ShaderPath, int Priority):PassRenderer(Dev,ShaderPath,Priority, PassRenderer::PassType::SHADER)
{
	Init(ShaderPath);
	CreateSignalBuffer();
...
void RayMarchingPass::DrawSetting(Microsoft::WRL::ComPtr<ID3D12GraphicsCommandList>& _CmdList, D3D12_VIEWPORT & _ViewPort, D3D12_RECT & _ScissorRect)
{
	//ルートシグネチャをセット
...
void RayMarchingPass::PassDraw(Microsoft::WRL::ComPtr<ID3D12GraphicsCommandList>& _CmdList)
{
	SetSignal();
...
void RayMarchingPass::Draw(Microsoft::WRL::ComPtr<ID3D12GraphicsCommandList>& _CmdList, D3D12_VIEWPORT & _ViewPort, D3D12_RECT & _ScissorRect, Microsoft::WRL::ComPtr<ID3D12DescriptorHeap>& _DepthBufferViewHeap)
{
	//ルートシグネチャをセット
	_CmdList->SetGraphicsRootSignature(_RootSigunature.Get());
...
void RayMarchingPass::PassClearAndSetRendererTarget(Microsoft::WRL::ComPtr<ID3D12GraphicsCommandList>& _CmdList, Microsoft::WRL::ComPtr<ID3D12DescriptorHeap>& _DepthStencilViewHeap, bool First)
{
...
void RayMarchingPass::ImGuiDrawBase()
{
}


bool RayMarchingPass::LoadShader(std::string ShaderPath)
{
	HRESULT Result = S_OK;
...
void RayMarchingPass::SetSignal()
{
	MappedSignal->OnDebug = Signals.OnDebug;
}
bool RayMarchingPass::CreateSignalBuffer()
{
	HRESULT Result = S_OK;

	D3D12_HEAP_PROPERTIES HeapProp = CD3DX12_HEAP_PROPERTIES(D3D12_HEAP_TYPE_UPLOAD);

	D3D12_RESOURCE_DESC ResDesc = CD3DX12_RESOURCE_DESC::Buffer(AligmentedValue(sizeof(RayMarchShaderSignal), D3D12_CONSTANT_BUFFER_DATA_PLACEMENT_ALIGNMENT));
...
bool RayMarchingPass::CreateRootSignature()
{
	HRESULT Result = S_OK;
...
bool RayMarchingPass::CreatePipeLineState()
{
	HRESULT Result = S_OK;
...
bool RayMarchingPass::Create_Rtv_Srv()
{
	return true;
}
void RayMarchingPass::AddBarrier(Microsoft::WRL::ComPtr<ID3D12GraphicsCommandList>& _CmdList, D3D12_RESOURCE_STATES StateBefore, D3D12_RESOURCE_STATES StateAfter)
{
